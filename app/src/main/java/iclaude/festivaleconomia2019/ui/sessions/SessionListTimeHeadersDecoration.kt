...
fun indexSessionHeaders(
    ...
    return sessions
        .mapIndexed { index, session ->
            index to zonedDateTimeFromTimestamp(context, session.startTimestamp)
        }
...
class ScheduleTimeHeadersDecoration(
    ...
    init {
        val attrs = context.obtainStyledAttributes(
            R.style.SessionListTimeHeaders,
    ...
    // possible improvement: let user pick between local or conference time zone (b/77606102). Show local for now.
    private val timeSlots: Map<Int, StaticLayout> =
        indexSessionHeaders(context, sessions, zoneId).map {
    ...
    override fun onDrawOver(c: Canvas, parent: RecyclerView, state: State) {
        ...
        for (i in parent.childCount - 1 downTo 0) {
            ...
            if (view == null) {
                ...
                Log.w(TAG, "View is null. Index: $i")
